...
- On branch `levelRender`, L3 toggles the native level renderer A/B mode. The native path currently disables the SNES backdrop priority band and emits SNES-equivalent native backdrop/BG1 pixels into custom slots with palette/layer metadata so the compositor can apply normal color math; the bottom-right overlay shows `SNES` or `NATIVE` next to the aim indicator.
- Native BG1 now overlays the stock SNES BG1 instead of removing it outright, so rooms whose BG1 tilemap/color-window behavior is not yet reproduced by the native path fall back to visible level geometry while still letting correct native pixels win.
